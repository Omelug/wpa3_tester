...
static void load_hwsim() {
	...
	if(hw_capabilities::run_cmd({ "modprobe", "mac80211_hwsim", "radios=2" }, nullopt, false) != 0) return;
	...
	if(g_hwsim_ok) atexit([] { hw_capabilities::run_cmd({ "modprobe", "-r", "mac80211_hwsim" }, nullopt, false); });
}
