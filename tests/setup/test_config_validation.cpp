...
TEST_CASE("RunStatus Config Validation - Test configuration") {
	...
	const vector<ConfigTestCase> tests = {
		...
		{ "8. netns with internal (valid)",
				"08_test_netns_internal_valid.yaml",
				"08_test_netns_internal_valid.yaml",
				true },
		...
		{ "14. extends as list (two bases merged)",
				"14_test_extends_list.yaml",
				"01_test_happy_path_minimal.yaml",
				true },
		{ "15. extends as list (current overrides base)",
				"15_test_extends_list_override.yaml",
				"01_test_happy_path_minimal.yaml",
				true },
...
TEST_CASE("RunStatus Config Validation - Validator configuration") {
	...
	const vector<ConfigTestCase> tests = {
		...
		{ "4. validator as list (single)",
				"04_test_validator_list_single.yaml",
				"01_result_validator_minimal.yaml",
				true },
		{ "5. validator as list (multi, all pass)",
				"05_test_validator_list_multi.yaml",
				"01_result_validator_minimal.yaml",
				true },
...
TEST_CASE("RunStatus Config Validation - Observer configuration") {
	...
	const vector<ConfigTestCase> tests = {
		{ "1. observer tcpdump valid",
				"01_test_observer_tcpdump_valid.yaml",
				"01_result_observer_tcpdump_valid.yaml",
				true },
		{ "2. observer tshark valid",
				"02_test_observer_tshark_valid.yaml",
				"02_result_observer_tshark_valid.yaml",
				true },
...
void check_dir_tree_structure(const path &expected_dir, const path &actual_dir) {
	...
	if(expected_tree != actual_tree) {
		for(const auto &path: expected_tree) {
			if(!actual_tree.contains(path)) { INFO("Missing in actual: " << path); }
		}
		for(const auto &path: actual_tree) {
			if(!expected_tree.contains(path)) { INFO("Extra in actual: " << path); }
		}
	}
...
TEST_CASE("RunStatus - parse_requirements()") {
	...
	SUBCASE("Parse actors and observers from config") {
		...
		rs.config()["observers"] = nlohmann::json{
			{ "tcpdump_observer", { { "program", "tcpdump" }, { "filter", "wlan.fc.type_subtype == 0x08" } } },
			{ "tshark_observer", { { "program", "tshark" }, { "filter", "eapol" } } }
		};
	...
	SUBCASE("Multiple actors parsing - actor_names") {
		...
		for(const auto &[actor_name, actor]: rs.actors) { CHECK_EQ(actor[SK::actor_name], actor_name); }
	}
}
...
TEST_CASE("RunStatus - Test suite test generation") {
	...
	const vector<ConfigSuiteCase> tests = { { "1. test suite minimal", "01_ts_path_minimal.yaml", "01_min" },
