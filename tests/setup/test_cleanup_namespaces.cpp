...
static bool netns_exists(const string &name) { return exists("/var/run/netns/" + name); }
...
static bool pid_alive(const pid_t pid) { return exists("/proc/" + to_string(pid)); }

struct NsGuard {
	string name;
	explicit NsGuard(string n):
		name(std::move(n)) {}
	~NsGuard() = default;
};
...
TEST_CASE("cleanup_all_namespaces - removes multiple empty namespaces") {
	...
	for(const auto &n: names) { CHECK_EQ(netns_exists(n), false); }
}
