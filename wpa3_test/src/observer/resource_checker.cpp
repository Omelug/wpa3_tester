...
void start_resource_monitoring(RunStatus &rs, const string &actor_name, const int interval_sec){
	...
	const vector<string> command = {
		"pidstat",
		//"-T", "CHILD",
		"-h", "-p", to_string(target_pid), "-u", // cpu statistic
		"-r",                                    // ram statistics
	...
	while(getline(infile, line)){
		if(line.find("[stdout]") != string::npos){
			...
			if(time_hhmmss.find(':') != string::npos){
				...
				if(tokens.size() > 14){
					outfile << timestamp_iso << "," << tokens[7] << "," << tokens[13] << "," << tokens[14] << endl;
				}
			}
		}
	}
...
void create_graph(const RunStatus &rs, const string &source, const G_elms &elements){
	const auto log_path = get_observer_folder(rs, "resource_checker") / ("ap" + SUFFIX_res + ".log");
