...
public:
	...
	std::map<std::string,std::optional<std::string>> str_con = {
		{"actor_name", std::nullopt}, {"source", std::nullopt}, {"iface", std::nullopt}, {"mac", std::nullopt},
		{"ssid", std::nullopt}, {"channel", std::nullopt}, {"signal", std::nullopt}, {"ht_mode", std::nullopt},
		{"driver", std::nullopt}, {"netns", std::nullopt}, {"sniff_iface", std::nullopt},
		// external whitebox only
		{"radio", std::nullopt}, // same like phy for this project
		{"whitebox_host", std::nullopt}, {"whitebox_ip", std::nullopt}, {"ssh_user", std::nullopt},
		{"ssh_port", std::nullopt}, {"ssh_password", std::nullopt}, {"external_OS", std::nullopt},
	};

	std::map<std::string,std::optional<bool>> bool_conditions = {
		{"AP", std::nullopt}, {"injection", std::nullopt}, {"control_monitor", std::nullopt},
		{"active_monitor", std::nullopt}, {"managed", std::nullopt}, {"monitor", std::nullopt},
		{"2_4GHz", std::nullopt}, {"5GHz", std::nullopt}, {"6GHz", std::nullopt}, {"WPA-PSK", std::nullopt},
		{"WPA3-SAE", std::nullopt}, {"80211n", std::nullopt}, {"80211ac", std::nullopt}, {"80211ax", std::nullopt},
