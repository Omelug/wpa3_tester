...
void start_remote(RunStatus &rs, const string &actor_name, const int interval_sec, const string &local_log){
	...
	const string script = "while true; do " "  count=$(iw dev " + iface + " station dump 2>/dev/null"
			"          | grep -c '^Station' || echo 0); " "  echo \"$(date +%s) $count\" >> " + remote_log + "; "
...
void start_local(RunStatus &rs, const string &actor_name, const string &iface, const int interval_sec){
	...
	const string script = "while true; do " "  count=$(iw dev " + iface + " station dump 2>/dev/null"
			"          | grep -c '^Station' || echo 0); " "  echo \"$(date +%s) $count\"; " "  sleep " +
			to_string(interval_sec) + "; " "done";

	const vector<string> cmd = {"sh", "-c", script};

	rs.process_manager.run(actor_name + SUFFIX_sta, cmd, {}, log_dir);
}
