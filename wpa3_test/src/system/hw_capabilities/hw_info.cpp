...
void HwInfo::from_json(const nlohmann::json &j) const {
	for(const auto sk: sk_keys()) {
		...
		if(j.contains(name) && j.at(name).is_string() && !j.at(name).get<string>().empty())
			actor->set(sk, j.at(name).get<string>());
	}
...
void Actor_config::load_hw_info(const optional<path> &cache) {
	...
	if(cache.has_value()) {
		...
		} catch(const exception &e) { log(LogLevel::WARNING, "get_hw_info: cache write failed: {}", e.what()); }
	}
}
}
