...
- most information are from https://github.com/vanhoefm/wifi-injection (check for more injection problems or better
  description)
...
- some network cards (Intel AC-3160 and those based on the RT5572 chipset) have issue with injecting fragments with the
...
- With the RT5572 chipset, this dummy frame must also have the same QoS TID as the injected frame, but all other fields
  did not matter

### Injection tests

- Acknowledgements in pure monitor mode
    - for testing needs to be AP around
