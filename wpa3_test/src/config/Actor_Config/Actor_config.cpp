...
string Actor_config::operator[](const string &key) const{
	if(const auto k = sk_cast(key); k.has_value()){
		const auto &v = (*this)[*k];
		if(!v.has_value()) throw config_err("Actor_config: key '" + key + "' has no value");
		return *v;
	}
...
bool Actor_config::matches(const Actor_config &offer) const{
	...
	return ranges::all_of(bk_keys(), [&](const auto k){
		const auto &required = (*this)[k];
...
Actor_config &Actor_config::operator+=(const Actor_config &other){
	for(const auto k: sk_keys()){
		...
		if(!mine.has_value()){
			...
			throw run_err(
				"Actor_config conflict on key '" + string(sk_name(k)) + "': '" + *mine + "' vs '" + *val + "'");
		}
	}
	...
	for(const auto k: bk_keys()){
		...
		if(!mine.has_value()){
			...
			throw run_err("Actor_config conflict on bool key '" + string(bk_name(k)) + "'");
		}
	}
	return *this;
}
...
string Actor_config::get(const SK key) const{
	...
	if(!v.has_value()) throw config_err("Actor_config: key '" + string(sk_name(key)) + "' has no value");
	return *v;
}

bool Actor_config::get(const BK key) const{
	const auto &v = (*this)[key];
	if(!v.has_value()) throw config_err("Actor_config: bool key '" + string(bk_name(key)) + "' has no value");
	return *v;
}
...
void Actor_config::print_ActorCMap(const string &title, const ActorCMap &actors){
	...
	for(const auto &[key, actor_ptr]: actors){
		...
		cout << "[" << key << "] " << (host.has_value() ? *host : "Actor_" + key) << " " << actor->to_str() << "\n";
	}
	cout << flush;
}
...
bool Actor_config::is_external_WB() const{
	return get(SK::source) == "external" && ((*this)[SK::whitebox_host].has_value() || (*this)[SK::whitebox_ip].
		has_value());
}

bool Actor_config::monitor_needed() const{
	return get_or(BK::monitor, false) || get_or(BK::active_monitor, false) || get_or(BK::control_monitor, false) ||
