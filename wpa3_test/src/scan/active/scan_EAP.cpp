...
enum class AuthStatus { UNKNOWN, IN_PROGRESS, SUCCESS, FAILED };
...
EAP_Info parse_eap_packet(const RawPDU &raw) {
	...
	static const map<uint8_t, string> method_names = {
		...
		{ TYPE_AKA, "EAP-AKA" },
		{ TYPE_PEAP, "EAP-PEAP" },
		{ TYPE_MSCHAPV2, "EAP-MSCHAPv2" },
		{ TYPE_POTP, "EAP-POTP" },
		{ TYPE_FAST, "EAP-FAST" },
		{ TYPE_EKE, "EAP-EKE" },
		{ TYPE_TEAP, "EAP-TEAP" },
		{ TYPE_AKA_PRIME, "EAP-AKA-PRIME" },
		{ TYPE_PWD, "EAP-PWD" },
...
static optional<monostate> handle_eap_pdu(
	...
	if(identity && session.identities.insert(*identity).second)
		log(LogLevel::INFO, "[*] New Identity for {}: {}", client_mac, *identity);

	if(method && session.methods.insert(*method).second)
		log(LogLevel::INFO, "[+] New Method for {}: {}", client_mac, *method);
