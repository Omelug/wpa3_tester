...
json collect_driver_specific(const string &driver_name, const string &phy) {
	...
	try {
		...
		if(d.find("mt76") != string::npos) {
			...
			j["family"] = "unknown";
			const path phy_root = path("/sys/kernel/debug/ieee80211") / (phy.rfind("phy", 0) == 0 ? phy : "phy" + phy);
			j["phy_debugfs_root"] = phy_root.string();
			j["available_subdirs"] = list_dir(phy_root);
		}
	} catch(const exception &e) { j["error"] = string("driver_specific collection failed: ") + e.what(); }
...
json collect_regulatory(const string &phy, const optional<string> &netns) {
	json j;
	try {
		j["iw_reg_get"] = hw_capabilities::run_cmd_output({ "iw", "reg", "get" }, netns);
	} catch(...) {}
	if(!phy.empty()) {
		const string phy_name = phy.rfind("phy", 0) == 0 ? phy : "phy" + phy;
		try {
			j["phy_reg_get"] = hw_capabilities::run_cmd_output({ "iw", "phy", phy_name, "reg", "get" }, netns);
		} catch(...) {}
