...
vector<DeauthTestEntry> DeauthTestEntry::collect_results(const path &test_data_dir) {
    ...
    ranges::sort(entries, [](const DeauthTestEntry &a, const DeauthTestEntry &b) {
        return tie(a.client_version, a.client_mfp, a.ap_mac) <
               tie(b.client_version, b.client_mfp, b.ap_mac);
    });
    return entries;
}

void DeauthTestEntry::render_table(overview::HtmlGuard &f, const string &title,
    const path &suite_data_dir, const path &page_dir, const string &t_name) {

    helper::div_card<DeauthTestEntry>(f, title, suite_data_dir, [&](overview::HtmlGuard &hg,
        const vector<DeauthTestEntry> &entries) {
