...
optional<EapPwdFrame> parse_eap_pwd(const vector<uint8_t>& eapol){
    ...
    f.pwd_data.assign(eapol.begin() + data_start, eapol.end());
    return f;
}
...
static vector<uint8_t> build_eapol_eap(const uint8_t code, uint8_t eap_id,
    ...
    const uint16_t eap_len  = static_cast<uint16_t>(EAP_HDR + eap_body.size());
...
bool do_auth(MonitorSocket& sock, const Channel& ch,
    ...
    while(steady_clock::now() < deadline){
        auto [pdu, raw] = sock.recv();
        if(!pdu) continue;
        const auto* resp = pdu->find_pdu<Dot11Authentication>();
        if(!resp) continue;
        if(resp->addr1() != our_mac) continue;
        if(resp->auth_seq_number() != 2) continue;
        if(resp->status_code() != 0){
            log(LogLevel::WARNING, "Auth rejected, status={}", resp->status_code());
            return false;
        }
...
bool do_assoc(MonitorSocket& sock, const Channel& ch,
              const HWAddress<6>& our_mac, const HWAddress<6>& ap_mac,
              string_view ssid, milliseconds timeout){
    ...
    while(steady_clock::now() < deadline){
        auto res = sock.recv();
        if(!res.pdu) continue;
        const auto* resp = res.pdu->find_pdu<Dot11AssocResponse>();
        if(!resp) continue;
        if(resp->addr1() != our_mac) continue;
        if(resp->status_code() != 0){
...
bool run_reflection_exchange(MonitorSocket& sock, const Channel& channel,
                              const HWAddress<6>& our_mac, const HWAddress<6>& ap_mac,
                              string_view ssid, string_view identity,
                              seconds timeout){
    ...
    enum class State { IDENTITY, PWD_ID, COMMIT, CONFIRM, DONE };
    auto state = State::IDENTITY;

    const auto deadline = steady_clock::now() + timeout;
    while(steady_clock::now() < deadline && state != State::DONE){
        auto [pdu, raw] = sock.recv();
        if(!pdu) continue;

        const auto eapol = extract_eapol(*pdu, our_mac);
        if(eapol.empty()) continue;
        ...
        case State::IDENTITY:{
            uint8_t eap_id = 0;
            if(!is_identity_request(eapol, eap_id)) continue;
            log(LogLevel::INFO, "EAP-Identity Request id={}", eap_id);
            send_eapol(sock, channel, our_mac, ap_mac,
                       build_identity_response(eap_id, identity));
            state = State::PWD_ID;
            break;
        }
        case State::PWD_ID:{
            auto f = parse_eap_pwd(eapol);
            if(!f || f->opcode != PWD_OPCODE_ID) continue;
            log(LogLevel::INFO, "EAP-PWD-ID Request");
            send_eapol(sock, channel, our_mac, ap_mac,
                       build_pwd_id_response(*f, identity));
            state = State::COMMIT;
            break;
        }
        case State::COMMIT:{
            auto f = parse_eap_pwd(eapol);
            if(!f || f->opcode != PWD_OPCODE_COMMIT) continue;
            log(LogLevel::INFO, "EAP-PWD-Commit Request – reflecting scalar+element");
            send_eapol(sock, channel, our_mac, ap_mac, reflect_commit(*f));
            state = State::CONFIRM;
            break;
        }
        case State::CONFIRM:{
            auto f = parse_eap_pwd(eapol);
            if(!f || f->opcode != PWD_OPCODE_CONFIRM) continue;
            log(LogLevel::INFO, "EAP-PWD-Confirm Request – reflecting confirm value");
            send_eapol(sock, channel, our_mac, ap_mac, reflect_confirm(*f));
            state = State::DONE;
            break;
        }
