...
namespace wpa3_tester::observer {
    ...
    void start_resource_monitoring_remote(RunStatus &rs, const string &actor_name, int interval_sec) {
        ...
        const string local_log = get_observer_folder(rs, string(program_name)) / (actor_name + "_resources.log");
        ...
        rs.process_manager.run(actor_name + "_res", ssh_command, get_observer_folder(rs, string(program_name)));
