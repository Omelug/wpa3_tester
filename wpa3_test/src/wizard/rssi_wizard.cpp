...
static void transmit_probe(const WifiSender &sender, const string &mac_str, bool is_5ghz = false) {
	...
	const uint8_t frame[] = {
		...
		0x00,
		0x00,
		0x00, // Present flags: IEEE80211_RADIOTAP_RATE
		rate,
		0x00, // valid rate
...
public:
	...
	void start() {
		running_ = true;
		worker_ = thread(&PcapSniffer::loop, this);
	}
	void stop() {
...
static void add_adapter(NetworkSetup &setup, const string &iface_name) {
	try {
		...
		if(!setup.nodes.contains(mac)) {
			...
			const double angle = 2.0 * M_PI * static_cast<double>(idx) / static_cast<double>(max(idx + 1, size_t{ 1 }));
	...
	} catch(const exception &e) { log(LogLevel::ERROR, "[!] Failed to add adapter {}: {}", iface_name, e.what()); }
}
	...
	while(g_running) {
		...
		for(auto it = setup.adapters.begin(); it != setup.adapters.end();) {
			...
			if(!alive) {
				...
				if(netns.has_value()) { hw_capabilities::run_cmd({ "ip", "netns", "del", *netns }, nullopt, false); }
		...
		for(const auto &a: setup.adapters) { present_macs.insert(a.actor.get(SK::mac)); }
...
static thread start_watcher(NetworkSetup &setup) { return thread(watcher_loop, ref(setup)); }
...
static RssiMatrix collect_rssi(const NetworkSetup &setup) {
	...
	for(const auto &src: setup.adapters) {
		for(const auto &rx: setup.adapters) {
			...
			if(src_mac != rx_mac) m[{ src_mac, rx_mac }] = setup.rssi_cache->get(src_mac, rx.actor.get(SK::iface));
		}
	}
	return m;
}
...
//FIXME source
static void smacof_step(map<HWAddress<6>, Node2D> &nodes, const vector<HWAddress<6>> &macs, const RssiMatrix &m) {
...
static void update_positions(
	...
	for(int i = 0; i < 5; ++i) smacof_step(nodes, macs, m);
}
...
static void render_text(
	...
	if(cond) {
		printf("---------------------------\n");
		for(const auto &[text, _]: cond->to_colored_parts(m)) printf("%s", text.c_str());
...
static FILE *init_gnuplot() {
	...
	if(getenv("DISPLAY") == nullptr) { return nullptr; }
...
bool run_rssi_wizard(const string &condition_str, const Channel &ch) {
	...
	if(!g_gnuplot_pipe) log(LogLevel::WARNING, "[!] gnuplot not available - using text output");
			...
			if(!setup->adapters.empty()) {
				...
				if(!g_paused) {
					...
					if(g_gnuplot_pipe) {
						...
						if(!render(g_gnuplot_pipe, setup->nodes, setup->adapters, m, status)) g_running = false;
					} else {
...
int main() {
	const string condition = "(00:c0:ca:b5:e1:58 <-> 90:de:80:6c:90:92) > -90 && "
