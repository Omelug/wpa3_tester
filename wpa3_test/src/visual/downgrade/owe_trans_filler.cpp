...
void generate_report(RunSuiteStatus &rss) {
	...
	report << "## Test Results\n\n";
	report << "| Test | AP Driver | Client Driver | Attacker Driver | BC probes | SSID probes | Disconnected | "
			  "Vulnerable |\n";
	report << "|------|-----------|---------------|-----------------|:---------:|:-----------:|:------------:|:--------"
			  "--:|\n";
	...
	for(const auto &e: entries) {
		...
		report << "| " << report::link(e.test_name, path(e.test_name) / REPORT_NAME) << " | " << e.ap_driver << " | "
			   << e.client_driver << " | " << e.attacker_driver << " | " << e.broadcast_probe_count << " | "
			   << e.ssid_probe_count << " | " << e.disconnected << " | " << vuln_link << " |\n";
	}
}
}