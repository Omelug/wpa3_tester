...
std::optional<bool> get_ocv(
		const RunStatus &rs, const std::string &actor_name); // wpa_supplicant: ocv (network block key)
...
std::string client_scanning_from_ap_log(
		const std::filesystem::path &ap_log, const Tins::HWAddress<6> &client_mac, TimeWindow window = {});
