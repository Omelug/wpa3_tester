...
namespace wpa3_tester::observer{
    ...
    void start_tcpdump_remote(RunStatus &run_status, const string &node_name, const string& filter) {
        const auto& actor = run_status.get_actor(node_name);
        // not /tmp so it will be valid after reboot (for failed tests) / DoS attacks
        const string remote_pcap = "/overlay/" + node_name + "_capture.pcap";
        const string local_pcap  = get_observer_folder(run_status, program_name) / (node_name + "_capture.pcap");
        ...
        // run via SSH
        auto conn = run_status.get_or_create_connection(node_name);
        conn.exec(cmd);

        // register cleanup + download on stop
        run_status.process_manager.on_stop(node_name + "_cap", [&conn, remote_pcap, local_pcap, node_name]() {
