...
TEST_CASE("ProcessManager - on_stop callback") {
	...
	pm.after_stop("test_proc", [&callback_called]() { callback_called = true; });
...
TEST_CASE("ProcessManager - get_pid") {
	...
	SUBCASE("non-existent process throws") { CHECK_THROWS_AS(pm.get_pid("ghost"), setup_err); }
...
TEST_CASE("ProcessManager - ignore_history") {
	...
	SUBCASE("non-existent process throws") { CHECK_THROWS_AS(pm.ignore_history("ghost"), setup_err); }
...
TEST_CASE("ProcessManager - discard_history") {
	...
	SUBCASE("non-existent process throws") { CHECK_THROWS_AS(pm.discard_history("ghost"), setup_err); }
	SUBCASE("existing process does not throw") {
		pm.run_dummy("proc");
