...
static bool parse_frame(PDU &pdu, ApInfoMap &ap_map, StaInfoMap &sta_map) {
	...
	if(const auto *data = pdu.find_pdu<Dot11Data>()) {
		...
		if(ap_map.contains(a2))
			try_add_sta(ap_map, sta_map, a2, a1);
		else if(ap_map.contains(a1))
			try_add_sta(ap_map, sta_map, a1, a2);
	}
	return false;
}
...
void run_attack(RunStatus &rs) {
	...
	for(const auto &entry: ap_map | views::values) save_external(entry.cfg);
...
void stats(const RunStatus &rs) {
	...
	for(const auto &ap_json: result.at("aps")) {
		...
		for(const auto &mac_j: ap_json.at("stations")) entry.stations.insert(hw_from_json(mac_j));
	}
