...
static Dot11Beacon build_legacy_beacon(const Dot11Beacon &src) {
    ...
    for (const auto &opt : src.options()) {
        switch (static_cast<Dot11::OptionTypes>(opt.option())) {
            ...
            case Dot11::TIM:
            case Dot11::RSN:
            case Dot11::SUPPORTED_OP_CLASSES:
                result.add_option(opt);
                break;
            default:
                break;  // drops VHT_CAP, VHT_OP, HT_*, ext-rates
        }
    }
    return result;
}

static void inject_legacy_beacons(const HWAddress<6> &ap_mac, const string &iface,
								  const int attack_time_sec,
								  const int ms_interval) {
