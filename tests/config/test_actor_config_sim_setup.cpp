...
public:
	...
	void set_channel(const Channel &ch) const override { calls.push_back("set_channel:" + to_string(ch.ch_num)); }

	bool called(const string &what) const { return ranges::find(calls, what) != calls.end(); }
};

// Minimal real_actor: provides iface + permanent_mac so setup_actor skips hw queries
static ActorPtr make_real(const string &iface = "wlan0", const string &mac = "aa:bb:cc:dd:ee:ff") {
