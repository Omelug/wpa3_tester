...
public:
	explicit HtmlPathTable(wpa3_tester::overview::HtmlGuard &hg, const std::vector<EntryType> &entries,
		...
		entries_(entries),
		t_name_(t_name) {
		if(t_name) { not_data_msg("Run " + t_name.value() + " first to get data"); }
	}
	...
	void add_column(std::string header, T EntryType::*member) {
		columns_.push_back({ std::move(header), [member, this](const EntryType &e) { hg_ << (e.*member); } });
	}
	...
	void add_rotated_column(std::string header, T EntryType::*member) {
		columns_.push_back({ std::move(header), [member, this](const EntryType &e) { hg_ << (e.*member); }, true });
	}
private:
	struct Column {
		std::string header;
	...
	std::string compute_column_prefix(const Column &col) const {
		...
		for(const auto &entry: entries_) { values.push_back(capture_evaluator_output(col, entry)); }
