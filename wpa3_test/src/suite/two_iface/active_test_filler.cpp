...
void generate_report(RunSuiteStatus &rss){
	...
	report <<
			"Tests whether a monitor-mode interface can both inject and receive ACKs (two-interface active TX test).\n\n";
	...
	report << "- Success Rate: " << fixed << setprecision(1) << (100.0 * static_cast<double>(passed_count) / static_cast
		<double>(entries.size())) << "%\n";

	report.close();
	set_public_perms(run_dir / REPORT_NAME);
	log(LogLevel::INFO, "Active test report generated: {}", run_dir / REPORT_NAME);
}
}
