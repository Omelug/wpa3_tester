...
namespace wpa3_tester::scan{
    ...
    bool parse_mgmt_frame(const Dot11ManagementFrame* mgmt, attack_scan::ScanAP &scan_ap) {
        ...
        }else if (mgmt->subtype() == 0 || mgmt->subtype() == 11) { // Assoc Req / Auth
    ...
    bool station_frame_parse(const unique_ptr<PDU> &pdu, attack_scan::ScanAP &scan_ap){ //const string& ap_mac, set<string>& found_stations) {
        if (!pdu) return false;
    ...
    void station_scan(attack_scan::ScanAP &scan_ap, const string& interface, const int timeout_sec, const filesystem::path &stations_pcap) {
