...
void create_state_log_graph(const path &state_log_path,
	...
	const bool has_times = ranges::all_of(
		transitions, [](const Transition &t) { return t.ts.has_value(); });
	...
	if (!gp) {
		log(LogLevel::ERROR, "state_log_graph: failed to open gnuplot");
		return;
	}

	fprintf(gp,
			"set terminal pngcairo size 1400,500 enhanced font 'Arial,10'\n");
