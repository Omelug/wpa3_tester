...
static vector<Token> tokenize(const string &s) {
	...
	while(i < s.size()) {
		...
		if(c == '!') {
			toks.push_back({ TokKind::Not, "!" });
			++i;
			continue;
		}
		if(c == '&' && i + 1 < s.size() && s[i + 1] == '&') {
			toks.push_back({ TokKind::And, "&&" });
			i += 2;
			continue;
		}
		if(c == '|' && i + 1 < s.size() && s[i + 1] == '|') {
			toks.push_back({ TokKind::Or, "||" });
			i += 2;
			continue;
		}
		if(c == '<') {
			if(i + 2 < s.size() && s[i + 1] == '-' && s[i + 2] == '>') {
				toks.push_back({ TokKind::Arrow, "<->" });
				i += 3;
				continue;
			}
			toks.push_back({ TokKind::Lt, "<" });
			++i;
			continue;
		}
		if(c == '>') {
			toks.push_back({ TokKind::Gt, ">" });
			++i;
			continue;
		}
...
struct Value {
	...
	[[nodiscard]] string to_str() const {
		if(is_const) {
			char buf[32];
			snprintf(buf, sizeof(buf), "%.0f", constant);
			return buf;
		}
...
struct CmpExpr: Expr {
	...
	[[nodiscard]] vector<pair<string, string>> to_colored_parts(const RssiMatrix &m) const override {
		if(!lhs.valid(m) || !rhs.valid(m)) return { { label + " (no data)", "#FFA500" } };
...
struct Parser {
	...
	Value parse_value() {
		if(peek().kind == TokKind::Number) return { true, consume().num, {} };
	...
	ExprPtr parse_atom() {
		if(peek().kind == TokKind::LParen) {
			if(pos + 1 < toks.size() && toks[pos + 1].kind == TokKind::Mac) {
				...
				if(peek().kind != TokKind::Lt && peek().kind != TokKind::Gt) throw runtime_error("expected '<' or '>'");
				consume();
	...
	ExprPtr parse_not() {
		if(peek().kind == TokKind::Not) {
			consume();
			return make_unique<NotExpr>(parse_not());
		}
		return parse_atom();
	}
	ExprPtr parse_and() {
		...
		while(peek().kind == TokKind::And) {
			consume();
			lhs = make_unique<AndExpr>(std::move(lhs), parse_not());
		}
		return lhs;
	}
	ExprPtr parse_expr() {
		...
		while(peek().kind == TokKind::Or) {
			consume();
			lhs = make_unique<OrExpr>(std::move(lhs), parse_and());
		}
		return lhs;
	}
};
