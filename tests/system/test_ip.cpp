...
TEST_CASE("get_ip - loopback returns 127.0.0.1") { CHECK_EQ(ip::get_ip("lo"), "127.0.0.1"); }

TEST_CASE("get_ip - nonexistent interface throws run_err") {
	CHECK_THROWS_AS(ip::get_ip("nonexistent_iface_xyz"), run_err);
}

TEST_CASE("resolve_host - localhost resolves to 127.0.0.1") { CHECK_EQ(ip::resolve_host("localhost"), "127.0.0.1"); }
