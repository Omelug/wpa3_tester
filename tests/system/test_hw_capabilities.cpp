...
struct GlobalConfigFixture {
	...
	GlobalConfigFixture():
		dir(current_path() / "tmp_hw_test_gc") {
...
TEST_CASE("freq_to_channel and channel_to_freq roundtrip") {
	SUBCASE("Roundtrip consistency") {
		vector<tuple<int, uint8_t, WifiBand>> test_cases = {
			{ 2412, 1, WifiBand::BAND_2_4 },
			{ 2437, 6, WifiBand::BAND_2_4_or_5 },
			{ 2472, 13, WifiBand::BAND_2_4 },
			// 2.4 GHz
			{ 5180, 36, WifiBand::BAND_5 },
			{ 5500, 100, WifiBand::BAND_2_4_or_5 },
			{ 5885, 177, WifiBand::BAND_5 }, // 5 GHz
			{ 5955, 1, WifiBand::BAND_6 },
			{ 6455, 101, WifiBand::BAND_6 },
			{ 7115, 233, WifiBand::BAND_6 } // 6 GHz
		};
...
TEST_CASE("hw_capabilities::list_interfaces - filter Loopback contains only lo") {
	...
	for(const auto &i: ifaces) CHECK_EQ(i.type, InterfaceType::Loopback);
}
